...
def get_line_trajectory(start_pose, end_pose, force_traj, start_time, T_traj) -> np.ndarray:
    ...
    Returns:
        np.ndarray: x*N array of points, where N is the length of force_traj and x the dimensionality of starting and end point
    """
...
if __name__ == "__main__":
    ...
    for i in range(4):
        ...
        x_ending[0] = x_center[0] + diamond_half_diagonal * np.cos(i * np.pi / 2 + np.pi)
        x_ending[2] = x_center[2] + diamond_half_diagonal * np.sin(i * np.pi / 2 + np.pi)
        t_start = iiwa.time.to_sec()
        points, time_vec = get_line_trajectory(pose_starting, x_ending, force_vec, t_start, T_traj)
    ...
    # Draw circle
    N = 80
    points = get_circle_points(x_center[[0, 2]], pose_starting[[0, 2]], N)
    pose_vec = np.ones((N, 7)) * x_starting
    pose_vec[:, 0] = points[0, :]
    pose_vec[:, 2] = points[1, :]
    t_start = iiwa.time.to_sec()
    time_vec = oc.Time.convert_double_to_time_vector(np.linspace(t_start, t_start + 4 * T_traj, N))
    force_vec = force_max * np.ones(N)
    iiwa.send_hybrid_force_motion_trajectory(pose_vec, force_vec, time_vec)
