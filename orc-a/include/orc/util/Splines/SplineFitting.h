/*
    ...
    Author: anonymous
    Date: Wed Apr 26 21:56:17 2023 +0200
...
namespace Eigen {
/**
 * \brief Implementation of std::merge.
 * \ingroup Splines_Module
 *
 * Custom implementation of std::merge, due to reduced std namespace in realtime environment
 *(twincat) \f{align*}
 *  u_0 & = \hdots = u_p = 0 \\
 *  u_{m-p} & = \hdots = u_{m} = 1 \\
 ...
 *
 * \param[in] first1, last1	-	the first range of elements to merge
 * \param[in] first2, last2	-	the second range of elements to merge
 * \param[in] d_first	-	the beginning of the destination range
 * \param[out] An output iterator to element past the last element copied.
 *
 * \sa https://en.cppreference.com/w/cpp/algorithm/merge
 **/
template <class InputIt1, class InputIt2, class OutputIt>
OutputIt custom_merge(InputIt1 first1, InputIt1 last1, InputIt2 first2, InputIt2 last2,
                      OutputIt d_first) {
 ...
 *
 * \sa Les Piegl and Wayne Tiller, The NURBS book (2nd ed.), 1997, 9.2.1 Global Curve Interpolation
 *to Point Data
 **/
...
void KnotAveragingWithDerivatives(const ParameterVectorType& parameters, const unsigned int degree,
    ...
    custom_merge(averageKnots.data(), averageKnots.data() + averageKnots.size(),
                 derivativeKnots.data(), derivativeKnots.data() + derivativeKnots.size(),
                 temporaryKnots.data());
    // temporaryKnots << averageKnots, derivativeKnots;
...
struct SplineFitting {
     ...
     * \param points The points for which an interpolating spline will be computed.
     * \param derivatives The desired derivatives of the interpolating spline at interpolation
     * points. \param derivativeIndices An array indicating which point each derivative belongs to.
     * This must be the same size as @a derivatives. \param degree The degree of the interpolating
     * spline. \param parameters The parameters corresponding to the interpolation points.
     *
     * \returns A spline interpolating @a points with @a derivatives at those points.
     *
     * \sa Les A. Piegl, Khairan Rajab, Volha Smarodzinana. 2008.
    ...
    /**
     * \brief Fits an interpolating spline to the given data points with up to C2 continuity.
     *
     * \param points The points for which an interpolating spline will be computed.
     * \param derivatives The desired derivatives of the interpolating spline at interpolation
     * points. \param derivativeIndices An array indicating which point each derivative belongs to.
     ...
     */
    template <typename PointArrayType, typename IndexArray>
    static SplineType InterpolateWithDerivatives2(const PointArrayType& points,
                                                  const PointArrayType& firstDerivatives,
                                                  const IndexArray& firstDerivativeIndices,
                                                  const PointArrayType& secondDerivatives,
                                                  const IndexArray& secondDerivativeIndices,
                                                  const unsigned int degree,
                                                  const ParameterVectorType& parameters);

    template <typename PointArrayType>
    static SplineType InterpolateWithEndDerivatives(const PointArrayType& points,
                                                    const PointArrayType& startDerivatives,
                                                    const PointArrayType& endDerivatives,
                                                    const unsigned int degree,
                                                    const ParameterVectorType& parameters);
};
