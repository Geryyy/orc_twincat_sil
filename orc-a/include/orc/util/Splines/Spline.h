...
namespace Eigen {
/**
 * \ingroup Custom Splines_Module
 * \class Spline
 * \brief A class representing multi-dimensional spline curves.
 * This is an extension to the Eigen unsupported/Spline module.
 *
...
public:
     ...
     * \param knots The underlying spline's knot vector.
     **/
    static BasisDerivativeType BasisFunctionDerivatives(const Scalar u, const DenseIndex order,
...
private:
    ...
    static void BasisFunctionDerivativesImpl(
        const typename Spline<_Scalar, _Dim, _Degree>::Scalar u, const DenseIndex order,
        const DenseIndex p, const typename Spline<_Scalar, _Dim, _Degree>::KnotVectorType& U,
        DerivativeType& N_);
};
...
DenseIndex Spline<_Scalar, _Dim, _Degree>::Span(
    ...
    if (u <= knots(0))
        return degree;
    const Scalar* pos =
        std::upper_bound(knots.data() + degree - 1, knots.data() + knots.size() - degree - 1, u);
...
template <typename _Scalar, int _Dim, int _Degree>
typename Spline<_Scalar, _Dim, _Degree>::PointType Spline<_Scalar, _Dim, _Degree>::operator()(
    Scalar u) const {
    ...
    const Block<const ControlPointVectorType, Dimension, Order> ctrl_pts(ctrls(), 0, span - p,
                                                                         Dimension, p + 1);
...
void derivativesImpl(const SplineType& spline, typename SplineType::Scalar u, DenseIndex order,
    ...
    typedef
        typename SplineTraits<SplineType, DerivativeOrder>::BasisDerivativeType BasisDerivativeType;
...
Spline<_Scalar, _Dim, _Degree>::BasisFunctionDerivatives(
    const typename Spline<_Scalar, _Dim, _Degree>::Scalar u, const DenseIndex order,
    const DenseIndex degree, const typename Spline<_Scalar, _Dim, _Degree>::KnotVectorType& knots) {
