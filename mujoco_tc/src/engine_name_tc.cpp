...
// TC: Added static_cast, as implicit conversion is not allowed in C++
int mj_name2id(const mjModel* m, int type, const char* name) {
  int mapadr;
  int* adr = 0;

  // get number of objects and name addresses
  int num = mjLOAD_MULTIPLE*_getnumadr(m, static_cast<mjtObj>(type), &adr, &mapadr);
...
const char* mj_id2name(const mjModel* m, int type, int id) {
  ...
  // get number of objects and name addresses
  int num = _getnumadr(m, static_cast<mjtObj>(type), &adr, &mapadr);
