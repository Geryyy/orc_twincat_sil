...
KsoeSlave::KsoeSlave(CTcTrace *trace, EventHandler* event_handler, int connection_id,void *input_buffer,void *output_buffer, PDSParameters* parameters, const AxisLimits* limits, const Axis* axis1) :
	KsoeSlave(trace, event_handler, connection_id, input_buffer, output_buffer, parameters, limits, axis1, NULL)
{
}

KsoeSlave::KsoeSlave(CTcTrace *trace, EventHandler* event_handler, int connection_id, void *input_buffer, void *output_buffer, PDSParameters* parameters, const AxisLimits* limits, const Axis* axis1, const Axis* axis2) {
  this->trace=trace;
  ...
  this->i2t_monitor1 = new I2tMonitor();
  if(type==KsoeSlaveType::PDS23) {
    ...
    this->i2t_monitor2 = new I2tMonitor();
  }
  else {
    ...
    this->i2t_monitor2 = NULL;
  }
  reset();

  this->position_soft_limit_output_timer[0] = 0;
  this->position_soft_limit_output_timer[1] = 0;
  this->position_soft_limit_output_timer_cleared[0] = 0;
  this->position_soft_limit_output_timer_cleared[1] = 0;

  this->velocity_limit_output_timer[0] = 0;
  this->velocity_limit_output_timer[1] = 0;
  this->velocity_limit_output_timer_cleared[0] = 0;
  this->velocity_limit_output_timer_cleared[1] = 0;

  this->i2t_limit_output_timer[0] = 0;
  this->i2t_limit_output_timer[1] = 0;
  this->i2t_limit_output_timer_cleared[0] = 0;
  this->i2t_limit_output_timer_cleared[1] = 0;

  this->current_limit_output_timer[0] = 0;
  this->current_limit_output_timer[1] = 0;
  this->current_limit_output_timer_cleared[0] = 0;
  this->current_limit_output_timer_cleared[1] = 0;

  this->motor_warning_i2t_output_timer[0] = 0;
  this->motor_warning_i2t_output_timer[1] = 0;
  this->motor_warning_i2t_output_timer_cleared[0] = 0;
  this->motor_warning_i2t_output_timer_cleared[1] = 0;

  this->motor_warning_temperature_output_timer[0] = 0;
  this->motor_warning_temperature_output_timer[1] = 0;
  this->motor_warning_temperature_output_timer_cleared[0] = 0;
  this->motor_warning_temperature_output_timer_cleared[1] = 0;

  this->sensor_torque_limit_output_timer[0] = 0;
  this->sensor_torque_limit_output_timer[1] = 0;
  this->sensor_torque_limit_output_timer_cleared[0] = 0;
  this->sensor_torque_limit_output_timer_cleared[1] = 0;

  this->invalid_motor_torque_output_timer[0] = 0;
  this->invalid_motor_torque_output_timer[1] = 0;
  this->invalid_motor_torque_output_timer_cleared[0] = 0;
  this->invalid_motor_torque_output_timer_cleared[1] = 0;

  this->motor_torque_limit_output_timer[0] = 0;
  this->motor_torque_limit_output_timer[1] = 0;
  this->motor_torque_limit_output_timer_cleared[0] = 0;
  this->motor_torque_limit_output_timer_cleared[1] = 0;

  this->error_output_timer[0] = 0;
  this->error_output_timer[1] = 0;
  this->error_output_timer_cleared[0] = 0;
  this->error_output_timer_cleared[1] = 0;

  this->saturation_output_timer_active[0] = 0;
  this->saturation_output_timer_active[1] = 0;
  this->saturation_output_timer_cleared[0] = 0;
  this->saturation_output_timer_cleared[1] = 0;

  this->runtime_error_output_timer = 0;
  this->runtime_error_output_timer_cleared = 0;
}
...
void KsoeSlave::reset() {
  ...
  axis1_sensor_i2t_limit = false;
  axis2_sensor_i2t_limit = false;
  axis1_control_current_limit = false;
  axis2_control_current_limit = false;
...
void KsoeSlave::statemachine(DWORD dc_time_L) {
  ...
  */
  ...
  if(last_error!=EventCode::NONE) return;
  ...
  if((input->Command&0xFE)==(unsigned char)KsoeState::RESET &&
    ...
    setLastError(EventCode::KsoeSlaveError_SLAVE_RESET);
  }
  ...
  //exception of the communication toggle bit, bit0 of the Command byte).
    ...
    setLastError(EventCode::KsoeSlaveError_WRONG_COMMAND);
  }
  else {
    ...
    switch(state) {
      ...
      case KsoeState::PROCESS_DATA:
        if(state_variable==-1) {
          ...
          if(input->VMT_Response!=(unsigned char)-(char)output->VMT_Challenge) {
            ...
            setLastError(EventCode::KsoeSlaveError_WRONG_VMT_RESPONSE);
            break;
          }
      ...
      default:
        ...
        setLastError(EventCode::KsoeSlaveError_UNKNOWN_STATE);
        break;
    }
...
void KsoeSlave::setState(KsoeState s) {
  trace->Log(tlVerbose, "%2d KsoeSlave::setState(): %s (0x%02x) -> %s (0x%02x)",connection_id,toString(state),state,toString(s),s);
  state=s;
}

void KsoeSlave::setMotionState1(MotionState s) {
  trace->Log(tlVerbose, "%2d KsoeSlave::setMotionState1(): %s (0x%02x) -> %s (0x%02x)",connection_id,toString(motion_state1),motion_state1,toString(s),s);
  motion_state1=s;
}

void KsoeSlave::setMotionState2(MotionState s) {
  trace->Log(tlVerbose, "%2d KsoeSlave::setMotionState2(): %s (0x%02x) -> %s (0x%02x)", connection_id, toString(motion_state2), motion_state2, toString(s), s);
  motion_state2=s;
}

void KsoeSlave::setMotionStateTarget(MotionState s) {
  if(s!=MotionState::MOTION) s=MotionState::STOP;
  trace->Log(tlVerbose, "%2d KsoeSlave::setMotionStateTarget(): 0x%02x -> 0x%02x",connection_id,motion_state_target,s);
...
void KsoeSlave::checkCrcReadSensor(AxisSensor *input1, AxisSensor *input2) {
  ...
  if(checkCrc()) {
    ...
    if(state==KsoeState::RESET) {
      ...
      input1->current_rms = 0;
      input1->i2t_percent = 0;
      if(pds23) {
        ...
        input2->current_rms = 0;
        input2->i2t_percent = 0;
      }
    }
    else {
      ...
      input1->current_rms = i2t_monitor1->updateAndOutput(input->Motor_torque_1) * axis1->current_resolution;
      input1->i2t_percent = input1->current_rms / axis1->holding_current;
      if(pds23) {
        ...
        input2->current_rms = i2t_monitor2->updateAndOutput(((PDS23_Tx*)input)->Motor_torque_2) * axis2->current_resolution;
        input2->i2t_percent = input2->current_rms / axis2->holding_current;
      }
      ...
      if(fabs_(input1->position)>(axis1->position_limit-limits->position_reserve)) {
		...
		KSOESLAVE_HANDLE_SENSOR_WARNING_FLAG("Position Soft Limit", axis1->axis_number, position_soft_limit_output_timer[0], position_soft_limit_output_timer_cleared[0], EventCode::KsoeSlaveWarning_POSITION_SOFT_LIMIT,
	  ...
	  KSOESLAVE_HANDLE_SENSOR_WARNING_FLAG("Position Soft Limit", axis2->axis_number, position_soft_limit_output_timer[1], position_soft_limit_output_timer_cleared[1], EventCode::KsoeSlaveWarning_POSITION_SOFT_LIMIT,
      ...
      if(fabs_(input1->position)>(axis1->position_limit-KSOESLAVE_POSITION_LIMIT_DISTANCE)) {
        ...
        if(input1->position>0&&input1->velocity_filtered>KSOESLAVE_VELOCITY_TOLERANCE||
          ...
          if(getMotionStateTarget()!=MotionState::STOP) {
			 trace->Log(tlError, "KsoeSlave::checkCrcReadSensor(): Motion aborted due to position limit in axis %d (%.2f �).", axis1->axis_number, AXIS_RAD2DEG(input1->position));
          }
          event_handler->set(axis1->axis_number, connection_id, EventCode::KsoeSlaveError_POSITION_LIMIT_HARD, EventType::ERROR);
          sensor_error=EventCode::KsoeSlaveError_POSITION_LIMIT_HARD;
        }
      }
      if(pds23 && fabs_(input2->position)>(axis2->position_limit-KSOESLAVE_POSITION_LIMIT_DISTANCE)) {
        ...
        if(input2->position>0&&input2->velocity_filtered>KSOESLAVE_VELOCITY_TOLERANCE||
          input2->position<0&&input2->velocity_filtered<-KSOESLAVE_VELOCITY_TOLERANCE) {
          if(getMotionStateTarget()!=MotionState::STOP) {
			  trace->Log(tlError, "KsoeSlave::checkCrcReadSensor(): Motion aborted due to position limit in axis %d (%.2f �).", axis2->axis_number, AXIS_RAD2DEG(input2->position));
          }
          event_handler->set(axis2->axis_number, connection_id, EventCode::KsoeSlaveError_POSITION_LIMIT_HARD, EventType::ERROR);
          sensor_error=EventCode::KsoeSlaveError_POSITION_LIMIT_HARD;
        }
      }
      //Check I2t monitoring limit
      KSOESLAVE_HANDLE_SENSOR_WARNING_FLAG("I2t Limit", axis1->axis_number, i2t_limit_output_timer[0], i2t_limit_output_timer_cleared[0], EventCode::KsoeSlaveWarning_I2T_LIMIT,
          input1->i2t_percent >= KSOELSAVE_I2TMONITOR_FACTOR,
          axis1_sensor_i2t_limit);
      KSOESLAVE_HANDLE_SENSOR_WARNING_FLAG("I2t Limit", axis2->axis_number, i2t_limit_output_timer[1], i2t_limit_output_timer_cleared[1], EventCode::KsoeSlaveWarning_I2T_LIMIT,
          pds23 && input2->i2t_percent >= KSOELSAVE_I2TMONITOR_FACTOR,
          axis2_sensor_i2t_limit);
      //Check velocity limit
	  KSOESLAVE_HANDLE_SENSOR_WARNING_FLAG("Velocity Limit", axis1->axis_number, velocity_limit_output_timer[0], velocity_limit_output_timer_cleared[0], EventCode::KsoeSlaveWarning_VELOCITY_LIMIT,
		  fabs_(input1->velocity_filtered) > axis1->velocity_limit*limits->velocity_limit_fraction,
        axis1_sensor_velocity_limit);
		KSOESLAVE_HANDLE_SENSOR_WARNING_FLAG("Velocity Limit", axis2->axis_number, velocity_limit_output_timer[1], velocity_limit_output_timer_cleared[1], EventCode::KsoeSlaveWarning_VELOCITY_LIMIT,
      ...
      //Check sensor torque limit
		KSOESLAVE_HANDLE_SENSOR_WARNING_FLAG("Sensor Torque Limit", axis1->axis_number, sensor_torque_limit_output_timer[0], sensor_torque_limit_output_timer_cleared[0], EventCode::KsoeSlaveWarning_SENSOR_TORQUE_LIMIT,
			fabs_(input1->torque) >= axis1->sensor_nominal_torque*limits->sensor_torque_limit_fraction,
        axis1_sensor_torque_limit);
		KSOESLAVE_HANDLE_SENSOR_WARNING_FLAG("Sensor Torque Limit", axis2->axis_number, sensor_torque_limit_output_timer[1], sensor_torque_limit_output_timer_cleared[1], EventCode::KsoeSlaveWarning_SENSOR_TORQUE_LIMIT,
    ...
    setLastError(EventCode::KsoeSlaveError_WRONG_CRC);
...
void KsoeSlave::statemachineWriteControlGenerateCrc(AxisControl *output1, AxisControl *output2, DWORD dc_time_L) {
  ...
  if (motion_watchdog1 >= KSOESLAVE_MOTION_WATCHDOG_TICKS) {
      trace->Log(tlError, "KsoeSlave::updateIOs(): Motion aborted by watchdog1 (Axis %d) in state %s (0x%02x), target %s (0x%02x).",
          axis1->axis_number, toString(motion_state1), motion_state1, toString(motion_state_target), motion_state_target);
      setLastError(EventCode::KsoeSlaveError_MOTION_WATCHDOG, axis1->axis_number);
  }
  if (pds23) {
      motion_watchdog2++;
      if (motion_watchdog2 >= KSOESLAVE_MOTION_WATCHDOG_TICKS) {
          trace->Log(tlError, "KsoeSlave::updateIOs(): Motion aborted by watchdog2 (Axis %d) in state %s (0x%02x), target %s (0x%02x).",
              axis2->axis_number, toString(motion_state2), motion_state2, toString(motion_state_target), motion_state_target);
          setLastError(EventCode::KsoeSlaveError_MOTION_WATCHDOG, axis2->axis_number);
      }
  }
  ...
  if(last_error==EventCode::NONE) {
    if(state!=KsoeState::PROCESS_DATA) {
      ...
      KSOESLAVE_HANDLE_ERROR_FLAG("Error SafeOs Runtime", 0, runtime_error_output_timer, runtime_error_output_timer_cleared, EventCode::KsoeSlaveError_SAFE_OS_RUNTIME,
      ...
      if(output1->motor_enable) {
        ...
        //Only check for errors if the corresponding motor is enabled.
          KSOESLAVE_HANDLE_ERROR_FLAG("Error static STO", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_STATIC_SAFE_TORQUE_OFF,
              input->Error_static_STO_1, output->Ack_error_static_STO_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("HS SBC", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_HS_SBC,
              input->Error_HS_SBC_1, output->Ack_error_HS_SBC_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("SBC", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_SBC,
              input->Error_SBC_1, output->Ack_error_SBC_1);
          //Handle motor-caused error flags
          KSOESLAVE_HANDLE_ERROR_FLAG("Motor error STO intern", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_MOTOR_SAFE_TORQUE_OFF_INTERN,
              input->Motor_error_STO_intern_1, output->Ack_Motor_error_STO_intern_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("Motor warning low voltage", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_MOTOR_WARNING_LOW_VOLTAGE,
              input->Motor_warning_low_voltage_1, output->Ack_Motor_warning_low_voltage_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("Motor error overvoltage", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_MOTOR_ERROR_OVER_VOLTAGE,
              input->Motor_error_over_voltage_1, output->Ack_Motor_error_over_voltage_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("Motor error internal", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_MOTOR_ERROR_INTERNAL,
              input->Motor_error_internal_1, output->Ack_Motor_error_internal_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("Motor error I2t", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_MOTOR_ERROR_I2T,
              input->Motor_error_I2t_1, output->Ack_Motor_error_I2t_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("Motor error overcurrent", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_MOTOR_ERROR_OVERCURRENT,
              input->Motor_error_overcurrent_1, output->Ack_Motor_error_overcurrent_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("Motor error total current", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_MOTOR_ERROR_TOTAL_CURRENT,
              input->Motor_error_total_current_1, output->Ack_Motor_error_total_current_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("Motor error temperature", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_MOTOR_ERROR_TEMPERATURE,
              input->Motor_error_temperature_error_1, output->Ack_Motor_error_temperature_error_1);
          KSOESLAVE_HANDLE_ERROR_FLAG("Motor error unkown command", axis1->axis_number,
              error_output_timer[0], error_output_timer_cleared[0], EventCode::KsoeSlaveError_MOTOR_ERROR_UNKNOWN_COMMAND,
              input->Motor_error_unknown_command_1, output->Ack_Motor_error_unknown_command_1);
          //Handle motor-caused warning flags
          KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Motor warning I2t", axis1->axis_number,
              motor_warning_i2t_output_timer[0], motor_warning_i2t_output_timer_cleared[0], EventCode::KsoeSlaveWarning_MOTOR_WARNING_I2T,
              input->Motor_warning_I2t_1, output->Ack_Motor_warning_I2t_1);
          KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Motor warning temperature", axis1->axis_number,
              motor_warning_temperature_output_timer[0], motor_warning_temperature_output_timer_cleared[0], EventCode::KsoeSlaveWarning_MOTOR_WARNING_TEMPERATURE,
              input->Motor_warning_temperature_warning_1, output->Ack_Motor_warning_temperature_warning_1);
        //Calculate the motor torque from the downthrust desired torque.
          KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Invalid Motor Torque", axis1->axis_number, invalid_motor_torque_output_timer[0], invalid_motor_torque_output_timer_cleared[0], EventCode::KsoeSlaveWarning_INVALID_MOTOR_TORQUE,
              isnan_(output1->motor_torque) || !isfinite_(output1->motor_torque),
              axis1_control_motor_torque_nan);
          //torque2current returns NaN if motor_torque is out of range
          current = torque2current(axis1, output1->motor_torque / axis1->gear_ratio);
          //Current is limited by the absolute values of maximum_current, torque_upper_limit and torque_lower_limit
          KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Current Limit", axis1->axis_number, current_limit_output_timer[0], current_limit_output_timer_cleared[0], EventCode::KsoeSlaveWarning_CURRENT_LIMIT,
              !axis1_control_motor_torque_nan && (isnan_(current) || current >= axis1->maximum_current * KSOELSAVE_CURRENT_LIMIT_FACTOR), axis1_control_current_limit);
          //Current is limited by the user-specified motor torque limit
          KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Motor Torque Limit", axis1->axis_number, motor_torque_limit_output_timer[0], motor_torque_limit_output_timer_cleared[0], EventCode::KsoeSlaveWarning_MOTOR_TORQUE_LIMIT,
              !axis1_control_motor_torque_nan && !axis1_control_current_limit && fabs_(current) >= axis1->nominal_current * limits->motor_torque_limit_fraction,
              axis1_control_motor_torque_limit);
          //Saturate the current
          if (!axis1_control_motor_torque_nan && !axis1_control_current_limit && !axis1_control_motor_torque_limit &&
              fabs_(current) >= (axis1->nominal_current * limits->motor_torque_saturation)) {
              current = axis1->nominal_current * limits->motor_torque_saturation * sgn_(current);
              if (!axis1_control_motor_torque_saturated) {
                  if (saturation_output_timer_active[0] == 0) {
                      saturation_output_timer_active[0] = OUTPUT_COOLDOWN_TIME;
                      trace->Log(tlWarning, "KsoeSlave::updateIOs(): Motor saturation of axis %d active.", axis1->axis_number);
                  }
              }
              ...
              if (axis1_control_motor_torque_saturated) {
                  if (saturation_output_timer_cleared[0] == 0) {
                      saturation_output_timer_cleared[0] = OUTPUT_COOLDOWN_TIME;
                      trace->Log(tlWarning, "KsoeSlave::updateIOs(): Motor saturation of axis %d cleared.", axis1->axis_number);
                  }
              }
              axis1_control_motor_torque_saturated = false;
          }
        ...
        if(motion_error!=EventCode::NONE || isnan_(current)) {
        ...
        switch(motion_state1) {
          case MotionState::STOP:
			...
			if (input->Motor_status_1 & 0x0008) {
				//((PDS23_Rx*)output)->Motor_control_1 = 0x0004 | 0x0080; // fault reset
				((PDS23_Rx*)output)->Motor_control_1 = 0x0080; // fault reset
				//feedMotionWatchdog1();
			}
      ...
      if(pds23) {
        if(output2->motor_enable) {
          //KSOESLAVE_HANDLE_ERROR_FLAG() sets local variable "motion_error", if an error is present
          //Only check for errors if the corresponding motor is enabled.
            KSOESLAVE_HANDLE_ERROR_FLAG("Error static STO", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_STATIC_SAFE_TORQUE_OFF,
                ((PDS23_Tx*)input)->Error_static_STO_2, ((PDS23_Rx*)output)->Ack_error_static_STO_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("HS SBC", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_HS_SBC,
                ((PDS23_Tx*)input)->Error_HS_SBC_2, ((PDS23_Rx*)output)->Ack_error_HS_SBC_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("SBC", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_SBC,
                ((PDS23_Tx*)input)->Error_SBC_2, ((PDS23_Rx*)output)->Ack_error_SBC_2);
            //Handle motor-caused error flags
            KSOESLAVE_HANDLE_ERROR_FLAG("Motor error STO intern", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_MOTOR_SAFE_TORQUE_OFF_INTERN,
                ((PDS23_Tx*)input)->Motor_error_STO_intern_2, ((PDS23_Rx*)output)->Ack_Motor_error_STO_intern_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("Motor warning low voltage", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_MOTOR_WARNING_LOW_VOLTAGE,
                ((PDS23_Tx*)input)->Motor_warning_low_voltage_2, ((PDS23_Rx*)output)->Ack_Motor_warning_low_voltage_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("Motor error overvoltage", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_MOTOR_ERROR_OVER_VOLTAGE,
                ((PDS23_Tx*)input)->Motor_error_over_voltage_2, ((PDS23_Rx*)output)->Ack_Motor_error_over_voltage_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("Motor error internal", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_MOTOR_ERROR_INTERNAL,
                ((PDS23_Tx*)input)->Motor_error_internal_2, ((PDS23_Rx*)output)->Ack_Motor_error_internal_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("Motor error I2t", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_MOTOR_ERROR_I2T,
                ((PDS23_Tx*)input)->Motor_error_I2t_2, ((PDS23_Rx*)output)->Ack_Motor_error_I2t_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("Motor error overcurrent", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_MOTOR_ERROR_OVERCURRENT,
                ((PDS23_Tx*)input)->Motor_error_overcurrent_2, ((PDS23_Rx*)output)->Ack_Motor_error_overcurrent_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("Motor error total current", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_MOTOR_ERROR_TOTAL_CURRENT,
                ((PDS23_Tx*)input)->Motor_error_total_current_2, ((PDS23_Rx*)output)->Ack_Motor_error_total_current_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("Motor error temperature", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_MOTOR_ERROR_TEMPERATURE,
                ((PDS23_Tx*)input)->Motor_error_temperature_error_2, ((PDS23_Rx*)output)->Ack_Motor_error_temperature_error_2);
            KSOESLAVE_HANDLE_ERROR_FLAG("Motor error unkown command", axis2->axis_number,
                error_output_timer[1], error_output_timer_cleared[1], EventCode::KsoeSlaveError_MOTOR_ERROR_UNKNOWN_COMMAND,
                ((PDS23_Tx*)input)->Motor_error_unknown_command_2, ((PDS23_Rx*)output)->Ack_Motor_error_unknown_command_2);
            //Handle motor-caused warning flags
            KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Motor warning I2t", axis2->axis_number,
                motor_warning_i2t_output_timer[1], motor_warning_i2t_output_timer_cleared[1], EventCode::KsoeSlaveWarning_MOTOR_WARNING_I2T,
                ((PDS23_Tx*)input)->Motor_warning_I2t_2, ((PDS23_Rx*)output)->Ack_Motor_warning_I2t_2);
            KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Motor warning temperature", axis2->axis_number,
                motor_warning_temperature_output_timer[1], motor_warning_temperature_output_timer_cleared[1], EventCode::KsoeSlaveWarning_MOTOR_WARNING_TEMPERATURE,
                ((PDS23_Tx*)input)->Motor_warning_temperature_warning_2, ((PDS23_Rx*)output)->Ack_Motor_warning_temperature_warning_2);
            //Calculate the motor torque from the downthrust desired torque.
            KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Invalid Motor Torque", axis2->axis_number, invalid_motor_torque_output_timer[1], invalid_motor_torque_output_timer_cleared[1], EventCode::KsoeSlaveWarning_INVALID_MOTOR_TORQUE,
            ...
            //Current is limited by the absolute values of maximum_current, torque_upper_limit and torque_lower_limit
            KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Current Limit", axis2->axis_number, current_limit_output_timer[1], current_limit_output_timer_cleared[1], EventCode::KsoeSlaveWarning_CURRENT_LIMIT,
                !axis2_control_motor_torque_nan && (isnan_(current) || current >= axis2->maximum_current * KSOELSAVE_CURRENT_LIMIT_FACTOR), axis2_control_current_limit);
            //Current is limited by the user-specified motor torque limit
            KSOESLAVE_HANDLE_MOTION_WARNING_FLAG("Motor Torque Limit", axis2->axis_number, motor_torque_limit_output_timer[1], motor_torque_limit_output_timer_cleared[1], EventCode::KsoeSlaveWarning_MOTOR_TORQUE_LIMIT,
                !axis2_control_motor_torque_nan && !axis2_control_current_limit && fabs_(current) >= axis2->nominal_current * limits->motor_torque_limit_fraction,
                axis2_control_motor_torque_limit);
            //Saturate the current
            if (!axis2_control_motor_torque_nan && !axis2_control_current_limit && !axis2_control_motor_torque_limit &&
                fabs_(current) >= (axis2->nominal_current * limits->motor_torque_saturation)) {
                current = axis2->nominal_current * limits->motor_torque_saturation * sgn_(current);
                if (!axis2_control_motor_torque_saturated) {
                    if (saturation_output_timer_active[1] == 0) {
                        saturation_output_timer_active[1] = OUTPUT_COOLDOWN_TIME;
                        trace->Log(tlWarning, "KsoeSlave::updateIOs(): Motor saturation of axis %d active.", axis2->axis_number);
                    }
                }
                ...
                if (axis2_control_motor_torque_saturated) {
                    if (saturation_output_timer_cleared[1] == 0) {
                        saturation_output_timer_cleared[1] = OUTPUT_COOLDOWN_TIME;
                        trace->Log(tlWarning, "KsoeSlave::updateIOs(): Motor saturation of axis %d cleared.", axis2->axis_number);
                    }
                }
                axis2_control_motor_torque_saturated = false;
            }
          ...
          switch(motion_state2) {
            case MotionState::STOP:
			  ...
			  if (((PDS23_Tx*)input)->Motor_status_2 & 0x0008) {
			    //((PDS23_Rx*)output)->Motor_control_2 = 0x0004 | 0x0080; // fault reset
				((PDS23_Rx*)output)->Motor_control_2 = 0x0080; // fault reset
				//feedMotionWatchdog2();
			  }
              else if(motion_state_target==MotionState::MOTION) {
...
EventCode KsoeSlave::getLastError() {
  return last_error;
}

EventCode KsoeSlave::getMotionError() {
  return motion_error;
}

EventCode KsoeSlave::getSensorError() {
  return sensor_error;
}

EventCode KsoeSlave::getMotionWarning() {
  return motion_warning;
}

EventCode KsoeSlave::getSensorWarning() {
  return sensor_warning;
}

void KsoeSlave::setLastError(EventCode error) {
    setLastError(error, 0);
}

void KsoeSlave::setLastError(EventCode error, int axis_number) {
  ...
  //output buffer and issues the reset command to the slave.
    trace->Log(tlVerbose, "%2d KsoeSlave::setLastError(): %s, Axis %d, (%d) -> %s (%d)", connection_id, axis_number, toString(last_error), (int)last_error, toString(error), (int)error);
    event_handler->set(axis_number, connection_id, error, EventType::ERROR);
...
double KsoeSlave::torque2current(const Axis* axis, double torque) {
    ...
    if (isnan_(torque) || !isfinite_(torque) ||
        torque > axis->torque_upper_limit || torque < axis->torque_lower_limit) {
        current = getnan_();
    }
    else {
        p = axis->a1 / axis->a3;
        q = -torque / (1 + axis->b1 * (motor_temperature - axis->T_ref)) / axis->a3;
...
double KsoeSlave::current2torque(const Axis* axis, double current) {
    return AXIS_CURRENT_TO_TORQUE(axis, current, motor_temperature);
}
...
void KsoeSlave::setMotorTemperature(USHORT motor_temperature) {
  ...
  if(motor_temperature>=273 && motor_temperature<333) {
    ...
    setLastError(EventCode::KsoeSlaveError_INVALID_TEMPERATURE);
  }
}
    ...
    if(output1->motor_enable) {
      ...
      //Check if velocity is within the tolerance band and stop this axis
        ...
        if(motion_state1==MotionState::MOTION) {
          setMotionState1(MotionState::DISENGAGING_MOTION);
          trace->Log(tlInfo, "KsoeSlave::performSoftStop(): Disabling motion on axis %d.", axis1->axis_number);
        }
      }
      else {
    ...
    if(output2->motor_enable) {
      ...
      //Check if velocity is within the tolerance band and stop this axis
        ...
        if(motion_state2==MotionState::MOTION) {
          setMotionState2(MotionState::DISENGAGING_MOTION);
          trace->Log(tlInfo, "KsoeSlave::performSoftStop(): Disabling motion on axis %d.", axis2->axis_number);
        }
      }
      else {
        //motor_torque already contains the gravitational torque. Additionally apply
...
// decrement timer every cycle or reset it
void KsoeSlave::timer_function(int &timer) {
	if (timer > 0) {
		timer--;
	}
	else {
...
void KsoeSlave::cooldownTimer() {
	timer_function(position_soft_limit_output_timer[0]);
	timer_function(position_soft_limit_output_timer[1]);
	timer_function(position_soft_limit_output_timer_cleared[0]);
	timer_function(position_soft_limit_output_timer_cleared[1]);

    timer_function(i2t_limit_output_timer[0]);
    timer_function(i2t_limit_output_timer[1]);
    timer_function(i2t_limit_output_timer_cleared[0]);
    timer_function(i2t_limit_output_timer_cleared[1]);

    timer_function(current_limit_output_timer[0]);
    timer_function(current_limit_output_timer[1]);
    timer_function(current_limit_output_timer_cleared[0]);
    timer_function(current_limit_output_timer_cleared[1]);

    timer_function(motor_warning_i2t_output_timer[0]);
    timer_function(motor_warning_i2t_output_timer[1]);
    timer_function(motor_warning_i2t_output_timer_cleared[0]);
    timer_function(motor_warning_i2t_output_timer_cleared[1]);

    timer_function(motor_warning_temperature_output_timer[0]);
    timer_function(motor_warning_temperature_output_timer[1]);
    timer_function(motor_warning_temperature_output_timer_cleared[0]);
    timer_function(motor_warning_temperature_output_timer_cleared[1]);

	timer_function(velocity_limit_output_timer[0]);
	timer_function(velocity_limit_output_timer[1]);
	timer_function(velocity_limit_output_timer_cleared[0]);
	timer_function(velocity_limit_output_timer_cleared[1]);

	timer_function(sensor_torque_limit_output_timer[0]);
	timer_function(sensor_torque_limit_output_timer[1]);
	timer_function(sensor_torque_limit_output_timer_cleared[0]);
	timer_function(sensor_torque_limit_output_timer_cleared[1]);

	timer_function(invalid_motor_torque_output_timer[0]);
	timer_function(invalid_motor_torque_output_timer[1]);
	timer_function(invalid_motor_torque_output_timer_cleared[0]);
	timer_function(invalid_motor_torque_output_timer_cleared[1]);

	timer_function(motor_torque_limit_output_timer[0]);
	timer_function(motor_torque_limit_output_timer[1]);
	timer_function(motor_torque_limit_output_timer_cleared[0]);
	timer_function(motor_torque_limit_output_timer_cleared[1]);

	timer_function(error_output_timer[0]);
	timer_function(error_output_timer[1]);
	timer_function(error_output_timer_cleared[0]);
	timer_function(error_output_timer_cleared[1]);

	timer_function(saturation_output_timer_active[0]);
	timer_function(saturation_output_timer_active[1]);
	timer_function(saturation_output_timer_cleared[0]);
	timer_function(saturation_output_timer_cleared[1]);

	timer_function(runtime_error_output_timer);
	timer_function(runtime_error_output_timer_cleared);
}
