...
FsoeSlave::FsoeSlave(CTcTrace *trace, EventHandler* event_handler, USHORT connection_id, USHORT slave_address, MFT_FSoE_Inputs *input_buffer, MFT_FSoE_Outputs *output_buffer, unsigned char *NSR_INDICATOR, unsigned char *NSR_INPUT) {
  this->trace=trace;
...
void FsoeSlave::setState(FsoeState s) {
  trace->Log(tlVerbose, "%2d FsoeSlave::setState(): %s (0x%02x) -> %s (0x%02x)", connection_id, toString(state), state, toString(s), s);
  this->state=s;
}
...
void FsoeSlave::statemachine() {
  ...
  if(input->FSOE_CMD!=output->FSOE_CMD) {
    ...
    setLastError(EventCode::FsoeSlaveError_WRONG_COMMAND);
  }
  else {
    switch(state) {
      ...
      default:
        ...
        setLastError(EventCode::FsoeSlaveError_UNKNOWN_STATE);
        break;
    }
  }
}
...
void FsoeSlave::updateIOs(MFTSensor *io) {
  if(state==FsoeState::PROCESS_DATA) {
    ...
    sensor_error=EventCode::NONE;
    FSOESLAVE_HANDLE_ERROR_FLAG("Button pressed", EventCode::FsoeSlaveError_BUTTON_PRESSED,
      input->SIND_ERR_3, output->SIND_ERR_ACK_3);
    FSOESLAVE_HANDLE_ERROR_FLAG("Button panic", EventCode::FsoeSlaveError_BUTTON_PRESSED,
  ...
  if(!hasValidInput()) {
    ...
    if(cycle_divider>=FSOESLAVE_CYCLE_DIVIDER) {
      ...
      if(checkCrc()) {
        ...
        setLastError(EventCode::FsoeSlaveError_WRONG_CRC);
      }
      generateCrc();
...
EventCode FsoeSlave::getLastError() {
  return last_error;
}

EventCode FsoeSlave::getSensorError() {
  return sensor_error;
}

void FsoeSlave::setLastError(EventCode error) {
  ...
  trace->Log(tlVerbose, "%2d FsoeSlave::setLastError(): %s (%d) -> %s (%d)", connection_id, toString(last_error), (int)last_error, toString(error), (int)error);
  event_handler->set(0, connection_id, error, EventType::ERROR);
...
void FsoeSlave::enableMotion() {
    setLEDState(LEDState::GREEN);
}

void FsoeSlave::disableMotion() {
